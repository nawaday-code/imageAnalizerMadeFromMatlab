...
% comment out for debug @2015-1214 by T.Saito
	...
	if num_of_files > 1 
        delete(hm);
        imaVOL = double(zeros(dcminfo.Rows ,dcminfo.Columns,num_of_files));
...
%             disp(dcminfo.SOPInstanceUID);
            ...
            if isfield(dcminfo,'RescaleSlope')
                imaVOL(:,:,dcminfo.InstanceNumber) = flipdim(squeeze(( ...
            ...
            if mod(i,3) == 0
                progbar(p.Number,round(i/num_of_files*100));drawnow;
            end
        end
        close(p.Number);
    else
